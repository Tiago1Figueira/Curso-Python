...
while True:
    horas = int(input('Insira a hora de início do evento:'))
    minutos = int(input('Insira os minutos de início do evento:'))
    segundos = int(input('Insira os segundos do evento:'))
    duracao = int(input('Insira a duração do evento em segundos:'))

    horas_i = horas * 3600
    minutos_i = minutos * 60
    segundos_i = segundos
    total_i = horas_i + minutos_i + segundos_i + duracao

    horas_f = int(total_i / 3600)
    rest_seg = total_i % 3600
    minutos_f = int(rest_seg / 60)
    segundos_f = rest_seg % 60
    print(f'O horário final do experimento começado as {horas}:{minutos}:{segundos} é {horas_f}:{minutos_f}:{segundos_f}!')
